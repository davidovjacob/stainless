...
const pkgImportPath = process.env['PKG_IMPORT_PATH'] ?? 'https://github.com/davidovjacob/stainless/';
...
async function postprocess() {
  for await (const file of walk(path.resolve(__dirname, '..', '..', 'dist'))) {
    ...
    if (file.endsWith('.d.ts')) {
      ...
      // if we have `import { type Readable } from 'https://github.com/davidovjacob/stainless/_shims/index'`,
